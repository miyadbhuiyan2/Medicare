...
public class AddSupplierController implements Initializable {
 ...
 ConnectDatabase connect2=new ConnectDatabase();
    ...
    {
      try{
          if(!(supplierName.getText().isEmpty()||contactNumber.getText().isEmpty()||companyAddress.getText().isEmpty()))
          {
              try{
                  query="INSERT INTO Supplier VALUES ('"+supplierName.getText()+"','"+contactNumber.getText()+"','"+companyAddress.getText()+"')";
          
            connect2.connectDBUpdate(query);
                
            query="SELECT Supplier_id from Supplier where Supplier_id=(SELECT max(Supplier_id) FROM Supplier);";
            ...
            alert.getDialogPane().setContentText("New Supplier has been added succefully");
